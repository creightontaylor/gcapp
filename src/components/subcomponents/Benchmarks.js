        ...
        if (this.props.pageSource !== 'Goal' && !this.props.calculateMatches) {
          ...
          .then((response) => {
            ...
            if (response.data.success) {
              ...
              this.setState({ benchmarks, animating: false })
              ...
              console.log('Benchmarks query did not work', error);
              this.setState({ animating: false })
          });
        }
        ...
        <View style={styles.bottompadding20}>
          ...
          {(this.state.animating) ? (
            ...
            <View style={styles.card}>
                {this.state.benchmarks.map((value, index) =>
                  <View key={index}>
                    <View>
                      <View style={styles.spacer} />

                      <View style={[styles.rowDirection]}>
                        <TouchableOpacity onPress={() => this.props.navigation.navigate('BenchmarkDetails', { selectedBenchmark: value })} style={[styles.rowDirection,styles.calcColumn110]}>
                          <View style={styles.width50}>
                            {(this.state.matchScores && this.state.matchScores[index]) ? (
                              ...
                              <Image source={{uri: benchmarksIconBlue }} style={[styles.square40,styles.contain,styles.topMargin5,styles.centerItem]}/>
                            )}
                            {(value.createdAt) && (
                              <View style={[styles.topPadding,styles.horizontalPadding5]}>
                                <Text style={[styles.descriptionText4,styles.descriptionTextColor,styles.boldText,styles.centerText]}>{convertDateToString(value.createdAt,"daysAgo")}</Text>
                              </View>
                            )}
                          </View>
